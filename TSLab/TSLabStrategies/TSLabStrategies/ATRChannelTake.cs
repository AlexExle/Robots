                    ...
                    if (bar > firstValidValue)
                    {
                        ...
                        foreach (var position in positions)
                        {
                            if (position.IsLong)
                            {
                                ...
                                if (positions.Count() < 2 && sec.Bars[bar - 1].High > lowLevelSeries2[bar])
                                    sec.Positions.SellIfLess(bar + 1, shares, lowLevelSeries2[bar], "OSell");
                            }
                            else
                            {
                                ...
                                if (positions.Count() < 2 && sec.Bars[bar - 1].Low < highLevelSeries2[bar])
                                    sec.Positions.BuyIfGreater(bar + 1, shares, highLevelSeries2[bar], "OBuy");
                            }
                        }                       
