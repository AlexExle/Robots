...
{
    public class ATRChannelForSmall : IExternalScript
    {
        ...
        public void Execute(IContext ctx, ISecurity sec)
        {
            ...
            for (int bar = 0; bar < sec.Bars.Count; bar++)
            {
                    ...
                    highLevelSeries2.Add(((calcPrice + (atr[bar] * multiplier.Value * 2)) / 10d)*10);
                    lowLevelSeries2.Add(((calcPrice - (atr[bar] * multiplier.Value * 2)) / 10d)*10);
