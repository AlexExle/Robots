...
{
    /// <summary>
    /// 96 Шаблонов-обвязок (версия с курса)
    /// 
    /// Код ТС: X.X.X.X
    /// 
    /// 0 - Короткая позиция        0 - Флэт        0 - Импульс                     0 - Трейлинг
    /// 1 - Длинная позиция         1 - Тренд       1 - Тренд и импульс             1 - Стоп и переворот
    /// 2 - Любая позиция                           2 - Тренд и откат               2 - Стоп/профит
    ///                                             3 - Тренд, откат и импульс      3 - Время
    /// </summary>
    class Binders64 : WealthScript
    {
        #region Типы
        /// <summary>
        /// Тип позиции
        ...
        StrategyParameter _positionType; // Длинная (1), короткая (0) или любая (2) позиция
        ...
        public Binders64()
        {
            ...
            _positionType = CreateParameter("Long", 2, 0, 2, 1);
        ...
        protected override void Execute()
        {
            ...
            PosType positionType = PosType.LongAndShort;
            switch (_positionType.ValueInt)
            {
                case 0:
                    positionType = PosType.Short;
                    break;
                case 1:
            ...
            for (int bar = firstValidValue; bar < Bars.Count; bar++) // Пробегаемся по всем барам
            {
                ...
                {
                        if (isSignalBuy && positionType != PosType.Short) // Если пришел сигнал на покупку и работаем с длинными позициями
                            BuyAtMarket(bar + 1, entryType.ToString());
                        else if (isSignalShort && positionType != PosType.Long) // Если пришел сигнал на короткую продажу и работаем с короткими позициями
