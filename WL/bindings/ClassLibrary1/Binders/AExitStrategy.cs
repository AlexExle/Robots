...
using ClassLibrary1.Binders.ExitBindings;
...
namespace ClassLibrary1.Binders
{
    ...
    public abstract class AExitStrategy
    {
        ...
        public static AExitStrategy CreateInstance(ClassLibrary1.CourseBinders64.ExitType exitType, WealthScript wlInstance, bool isTrend)
        {
            switch (exitType)
            {
                case CourseBinders64.ExitType.TrailingStop:
                    return new TrailingStop(wlInstance, isTrend);                  
                case CourseBinders64.ExitType.StopAndReverse:
                    return new StopAndReverse(wlInstance, isTrend);                  
                case CourseBinders64.ExitType.StopAndProfit:
                    return new StopAndProfit(wlInstance, isTrend);                    
                case CourseBinders64.ExitType.Time:
